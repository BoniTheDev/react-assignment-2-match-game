...
class AppStore extends Component {
  ...
  playAgainTheGame = () => {
    this.setState({isGameOver: false, timer: 60, counter: 0})
    this.componentDidMount()
  }

  componentDidMount = () => {
    this.intervalId = setInterval(this.timeLapse, 1000)
  }
  ...
  getThumbnailId = id => {
    const {matchImageDetails, isGameOver} = this.state

    if (matchImageDetails.id === id) {
      this.setState(prevState => ({
        counter: prevState.counter + 1,
      }))
      const randomImageDetails =
        imagesList[Math.ceil(Math.random() * imagesList.length - 1)]
      this.setState({matchImageDetails: randomImageDetails})
    } else {
      this.setState(prevState => ({
        counter: prevState.counter,
  ...
  getFilterThumbnailsByActiveTabId = () => {
    const {activeTabId} = this.state

    const filterThumbNails = imagesList.filter(
  ...
  render() {
    ...
    const {imageUrl, thumbnailUrl} = matchImageDetails

    const filterdListbyTabId = this.getFilterThumbnailsByActiveTabId()

    return (
      <div className="match-app-container">
        <nav className="match-nav-container">
          <ul className="match-nav-container">
            ...
            <div className="score-timer-card">
              <p className="score-text">
                Score: <span className="score-count">{counter}</span>
              </p>
          ...
          {isGameOver ? (
            ...
            <>
              ...
              <ul className="thumbnails-list">
                {filterdListbyTabId.map(eachImage => (
                  <ImageItem
